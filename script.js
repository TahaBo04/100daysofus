...
/* Personalize text */
[herNameDisplay, herNameFooter, herNameLetter, herNameBadge].forEach(el => el.textContent = HER_NAME);
myNameEls.forEach(el => el && (el.textContent = YOUR_NAME));
...
streakNumber.textContent = `Day ${diffDays} ❤️`;
...
lightbox.addEventListener("click", (e)=> { if (e.target === lightbox) lightbox.setAttribute("hidden",""); });
...
confettiBtn.addEventListener("click", ()=> fireConfetti(240));
